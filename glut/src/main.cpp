...
// float buffering, Z buffering, and texturing.
//
...
{
public:
	Transform(glm::vec3 x, glm::quat q):
		_M_x(x),
		_M_q(q)
	{}
	glm::mat4	mat4_cast()
	{
		glm::mat4 ret;
		return glm::translate(ret, _M_x) * glm::mat4_cast(_M_q);
		//return glm::mat4_cast(_M_q) * glm::translate(ret, _M_x);
		//return glm::mat4_cast(glm::conjugate(_M_q)) * glm::translate(ret, _M_x);
		//return glm::translate(ret, _M_x) * glm::mat4_cast(glm::conjugate(_M_q));
	}
	glm::vec3	_M_x;
	glm::quat	_M_q;
};
float red[]    = {1.0, 0.0, 0.0, 1.0};
float yellow[] = {0.5, 0.0, 0.5, 1.0};
float blue[]   = {0.0, 0.0, 1.0, 1.0};
float green[]  = {0.0, 1.0, 0.0, 1.0};
float cyan[]   = {0.0, 0.5, 0.5, 1.0};
...
void RenderObjects(void)
{
	...
	//Transform t(telem->x_[ti], glm::conjugate(telem->q_[ti]));
	Transform t(telem->x_[ti], telem->q_[ti]);
	
	glm::mat4 m = t.mat4_cast();
...
void load() {
	//const char * filename = "data/telem.txt";
